...
private let backgroundRefreshIdentifier = "com.terrace.background-generation.refresh"
private let backgroundProcessingIdentifier = "com.terrace.background-generation.processing"
...
private final class IosBackgroundGenerationHandler {
  ...
  private func start(arguments: Any?, result: @escaping FlutterResult) {
    ...
    if args["liveActivityEnabled"] as? Bool ?? false {
      startLiveActivity(
        title: args["title"] as? String ?? "Terrace",
  ...
  private func finish(arguments: Any?, result: @escaping FlutterResult) {
    let args = arguments as? [String: Any] ?? [:]
    let title = args["title"] as? String ?? "Terrace"
  ...
  private func cancel(arguments: Any?, result: @escaping FlutterResult) {
    let args = arguments as? [String: Any] ?? [:]
    finishLiveActivity(
      title: liveActivityDisplayTitle.isEmpty ? "Terrace" : liveActivityDisplayTitle,
      detail: args["detail"] as? String ?? ""
    )
    endBackgroundTask()
    resetGenerationOptions()
  ...
  private func beginBackgroundTask() {
    ...
    backgroundTask = UIApplication.shared.beginBackgroundTask(withName: "TerraceBackgroundGeneration") { [weak self] in
  ...
  private func scheduleBackgroundTasks() {
    ...
    do {
      ...
      NSLog("Terrace background refresh schedule failed: \(error)")
    }
    ...
    do {
      ...
      NSLog("Terrace background processing schedule failed: \(error)")
    }
  }
  ...
  private func showCompletionNotification(title: String, body: String) {
    ...
    let request = UNNotificationRequest(identifier: "terrace.background-generation.\(Date().timeIntervalSince1970)", content: content, trigger: nil)
    UNUserNotificationCenter.current().add(request)
  }

  private func isLiveActivityActive() -> Bool {
    if #available(iOS 16.1, *) {
      return liveActivity as? Activity<TerraceGenerationActivityAttributes> != nil
    }
    return false
  }
    ...
    if #available(iOS 16.1, *) {
      ...
      do {
        if #available(iOS 16.2, *) {
          liveActivity = try Activity<TerraceGenerationActivityAttributes>.request(attributes: TerraceGenerationActivityAttributes(title: title), content: ActivityContent(state: state, staleDate: nil), pushType: nil)
        } else {
          liveActivity = try Activity<TerraceGenerationActivityAttributes>.request(attributes: TerraceGenerationActivityAttributes(title: title), contentState: state, pushType: nil)
        }
        startLiveActivityRefreshTimer()
      } catch {
        NSLog("Terrace live activity start failed: \(error)")
        liveActivity = nil
      }
    }
  }
  ...
  private func markLiveActivityFinished(title: String, detail: String) {
    if #available(iOS 16.1, *), let activity = liveActivity as? Activity<TerraceGenerationActivityAttributes> {
      let finishedAt = Date()
      liveActivityDisplayTitle = title
      liveActivityDetail = detail
  ...
  private func refreshLiveActivity() {
    guard #available(iOS 16.1, *), let activity = liveActivity as? Activity<TerraceGenerationActivityAttributes> else { return }
    guard !liveActivityFinished else { return }
  ...
  private func liveActivityState(
    ...
    finishedAt: Date?,
    isFinished: Bool
  ) -> TerraceGenerationActivityAttributes.ContentState {
    let startedAt = liveActivityStartedAt
    let effectiveFinishedAt = finishedAt ?? Date()
    return TerraceGenerationActivityAttributes.ContentState(
