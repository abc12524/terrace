...
import 'terrace_fetch_server.dart';
...
/// Start the in-memory @terrace/fetch MCP server and connect a client to it.
...
startFetchMcpInMemory() async {
  final server = TerraceFetchMcpServerEngine();
  final transport = TerraceInMemoryClientTransport(server);

  final client = mcp.McpClient.createClient(
    mcp.McpClient.simpleConfig(name: 'Terrace App', version: '1.0.0'),
  );
...
Future<List<(mcp.Tool tool, String id)>> listFetchTools(
  ...
  const serverName = '@terrace/fetch';
  return tools
