...
/// @terrace/fetch — In-memory MCP server engine and transport (Flutter/Dart)
///
/// Provides four tools:
...
class TerraceFetchRequestPayload {
  final Uri url;
  final Map<String, String> headers;

  TerraceFetchRequestPayload({required this.url, Map<String, String>? headers})
    : headers = headers ?? const {};

  static TerraceFetchRequestPayload parse(Object? args) {
    if (args is! Map) {
      throw ArgumentError(
    ...
    return TerraceFetchRequestPayload(url: uri, headers: headers);
  }
}

class TerraceFetcher {
  static const _defaultUA =
      'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';

  static Future<http.Response> _fetch(TerraceFetchRequestPayload payload) async {
    try {
  ...
  static Future<Map<String, dynamic>> html(
    TerraceFetchRequestPayload payload,
  ) async {
    try {
...
/// Minimal JSON-RPC server for MCP that serves @terrace/fetch tools.
class TerraceFetchMcpServerEngine {
  bool _closed = false;
      ...
      switch (method) {
        case mcp.McpProtocol.methodInitialize:
          return _ok(
            id,
            result: {
              'serverInfo': {'name': '@terrace/fetch', 'version': '0.1.0'},
        ...
        case mcp.McpProtocol.methodCallTool:
          ...
          TerraceFetchRequestPayload payload;
          try {
            payload = TerraceFetchRequestPayload.parse(arguments);
          } catch (e) {
            return _ok(id, result: TerraceFetcher._err(e.toString()));
          }

          if (name == 'fetch_html') {
            return _ok(id, result: await TerraceFetcher.html(payload));
          }
          if (name == 'fetch_markdown') {
            return _ok(id, result: await TerraceFetcher.markdown(payload));
          }
          if (name == 'fetch_txt') {
            return _ok(id, result: await TerraceFetcher.txt(payload));
          }
          if (name == 'fetch_json') {
            return _ok(id, result: await TerraceFetcher.json(payload));
          }
...
class TerraceInMemoryClientTransport implements mcp.ClientTransport {
  final TerraceFetchMcpServerEngine _server;
  final _messageController = StreamController<dynamic>.broadcast();
  final _closeCompleter = Completer<void>();
  bool _closed = false;

  TerraceInMemoryClientTransport(this._server);
