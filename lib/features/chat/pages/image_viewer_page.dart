...
class _ImageViewerPageState extends State<ImageViewerPage>
  ...
  Future<void> _saveCurrent() async {
    ...
    try {
      ...
      final name = 'terrace-${DateTime.now().millisecondsSinceEpoch}';
      ...
      if (src.startsWith('data:')) {
        ...
        if (i != -1) {
          ...
          temp = await File(
            p.join(
              tmp.path,
              'terrace_${DateTime.now().millisecondsSinceEpoch}.png',
            ),
        ...
        if (resp.statusCode >= 200 && resp.statusCode < 300) {
          ...
          temp = await File(
            p.join(
              tmp.path,
              'terrace_${DateTime.now().millisecondsSinceEpoch}${ext.isNotEmpty ? ext : '.jpg'}',
            ),
          ).create(recursive: true);
    ...
    if (!ok) {
      try {
        ...
        if (path == null) {
          ...
          path = p.join(
            dir.path,
            'terrace_clip_${DateTime.now().millisecondsSinceEpoch}$ext',
          );
  ...
  Future<void> _saveCurrentDesktop() async {
    ...
    try {
      ...
      final defaultName = 'terrace-${DateTime.now().millisecondsSinceEpoch}$ext';
