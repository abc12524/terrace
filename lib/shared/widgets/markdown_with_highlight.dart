...
import 'package:Terrace/theme/theme_factory.dart' show getPlatformFontFallback;
import 'package:provider/provider.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:flutter_math_fork/flutter_math.dart';
import '../../core/providers/settings_provider.dart';
import 'package:Terrace/desktop/html_preview_dialog.dart';
...
class _MarkdownTableBlock extends StatelessWidget {
  ...
  Future<bool> _savePngBytesToGallery(Uint8List bytes) async {
    final result = await ImageGallerySaverPlus.saveImage(
      bytes,
      quality: 100,
      name: 'terrace-table-${DateTime.now().millisecondsSinceEpoch}',
    );
    if (result is Map) {
  ...
  Future<File> _writeTableImageTempFile(Uint8List bytes) async {
    ...
    final file = File(
      p.join(
        dir.path,
        'terrace-table-${DateTime.now().millisecondsSinceEpoch}.png',
      ),
    );
  ...
  Future<bool> _writePngToClipboard(Uint8List bytes) async {
    try {
      ...
      if (clipboard != null) {
        final item = DataWriterItem(suggestedName: 'terrace-table.png');
  ...
  Future<bool> _saveCachedMermaidPng(Uint8List bytes) async {
    try {
      ...
      final result = await ImageGallerySaverPlus.saveImage(
        bytes,
        quality: 100,
        name: 'terrace-mermaid-${DateTime.now().millisecondsSinceEpoch}',
      );
      if (result is Map) {
        final isSuccess =
