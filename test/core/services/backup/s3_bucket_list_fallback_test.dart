...
import 'package:Terrace/core/models/backup.dart';
import 'package:Terrace/core/services/backup/s3_client.dart';
...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  <Contents>
    <Key>terrace_backups/terrace_backup_2026-04-22T10-11-12.123456.zip</Key>
...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  <Contents>
    <Key>terrace_backups/terrace_backup_2026-04-20T09-00-00.123456.zip</Key>
...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  <Contents>
    <Key>terrace_backups/backup_3.zip</Key>
...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  ...
  <Contents>
    <Key>terrace_backups/page_1.zip</Key>
...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  ...
  <Contents>
    <Key>terrace_backups/page_2.zip</Key>
    <LastModified>2026-04-22T12:00:00.000Z</LastModified>
    <Size>222</Size>
  </Contents>
</ListBucketResult>''';
...
String _manifestJson() {
  ...
  "items": [
    {
      "key": "terrace_backups/terrace_backup_2026-04-22T10-11-12.123456.zip",
      "displayName": "terrace_backup_2026-04-22T10-11-12.123456.zip",
      "size": 128,
...
void main() {
  group('S3 bucket list fallback', () {
    test('test() succeeds when manifest key is missing', () async {
      ...
      expect(seenPaths, [
        '/backup-bucket/terrace_backups/.terrace_backups_manifest.json',
      ]);
    });
    ...
    test(
      ...
      () async {
        ...
        expect(items.map((e) => e.displayName).toList(), [
          'terrace_backup_2026-04-20T09-00-00.123456.zip',
        ]);
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          containsPair('key', 'terrace_backups/backup_3.zip'),
        );
  ...
  <Message>Access Denied</Message>
    ...
    test('uploadFile writes manifest object after upload', () async {
      ...
      server.listen((request) async {
        ...
        if (request.method == 'GET' &&
          ...
          request.response.write(_noSuchKeyXml());
            ...
            request.uri.path == '/backup-bucket/terrace_backups/demo.zip') {
          await request.drain<void>();
          request.response.statusCode = HttpStatus.ok;
        } else if (request.method == 'PUT' &&
            request.uri.path ==
      ...
      await const S3BackupClient().uploadFile(
        _config(server),
        key: 'terrace_backups/demo.zip',
        file: file,
      );

      expect(
        seenPaths,
        contains('/backup-bucket/terrace_backups/.terrace_backups_manifest.json'),
      );
      expect(manifestBody, contains('"key":"terrace_backups/demo.zip"'));
    });
    ...
    test(
      ...
      () async {
        ...
        expect(seenPaths, contains('/backup-bucket/terrace_backups/demo.zip'));
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          'terrace_backup_2026-04-22T10-11-12.123456.zip',
        );
      },
    );
  ...
  <Message>Access Denied</Message>
    ...
    test('endpoint that already includes bucket is not duplicated', () async {
      ...
      expect(
        ...
        containsAll([
          '/backup-bucket/terrace_backups/.terrace_backups_manifest.json',
          '/backup-bucket/terrace_backups/demo.zip',
          '/backup-bucket',
        ]),
      );
      ...
      expect(
        paths,
        isNot(contains('/backup-bucket/backup-bucket/terrace_backups/demo.zip')),
      );
    });
  });
}
